...
import { Rook } from "./figures/Rook"
    ...
    export class Cell{
        ...
        blackKingKill(cell: Cell, kingParams: XY, whiteKing: Cell | null){
            if(cell.figure?.figureName === FigureName.QUEEN){
                if(this.tryToKillKingHorizontal(cell, kingParams, whiteKing) || this.tryToKillKingDiagonal(cell, kingParams, whiteKing) || this.tryToKillKingVertical(cell, kingParams, whiteKing)){
                    return true;
                }
            }
            if(cell.figure?.figureName === FigureName.BISHOP){
                if(this.tryToKillKingDiagonal(cell, kingParams, whiteKing)){
                    return true
                }
            }
            if (cell.figure?.figureName === FigureName.KNIGHT) {
                if(this.tryToKillKingWithKnight(cell, kingParams, whiteKing)){
                    return true
                }
            }
            if(cell.figure?.figureName === FigureName.PAWN){
                if(this.tryToKillKingWithPawn(cell, kingParams, whiteKing)){
                    return true
                }
            }
            if(cell.figure?.figureName === FigureName.ROOK){
                if(this.tryToKillKingHorizontal(cell, kingParams, whiteKing) || this.tryToKillKingVertical(cell, kingParams, whiteKing)){
                    return true
                }
            }
        }
        
        whiteKingKill(cell: Cell, kingParams: XY, blackKing: Cell | null){
            if(cell.figure?.figureName === FigureName.QUEEN){
                if(this.tryToKillKingHorizontal(cell, kingParams, blackKing) || this.tryToKillKingDiagonal(cell, kingParams, blackKing) || this.tryToKillKingVertical(cell, kingParams, blackKing)){
                    return true;
                }
            }
            if(cell.figure?.figureName === FigureName.BISHOP){
                if(this.tryToKillKingDiagonal(cell, kingParams, blackKing)){
                    return true
                }
            }
            if (cell.figure?.figureName === FigureName.KNIGHT) {
                if(this.tryToKillKingWithKnight(cell, kingParams, blackKing)){
                    return true
                }
            }
            if(cell.figure?.figureName === FigureName.PAWN){
                if(this.tryToKillKingWithPawn(cell, kingParams, blackKing)){
                    return true
                }
            }
            if(cell.figure?.figureName === FigureName.ROOK){
                if(this.tryToKillKingHorizontal(cell, kingParams, blackKing) || this.tryToKillKingVertical(cell, kingParams, blackKing)){
                    return true
                }
            }
        }

        tryToKillKingDiagonal(cell: Cell, kingParams: XY, king: Cell | null){        
            const dx = Math.abs(king?.x - kingParams.x);
            const dy = Math.abs(king?.y - kingParams.y);

            const dxKing = Math.abs(cell.x - kingParams.x);
