...
export class Cell{
        ...
        blockFigureMovementDiagonal(figure: Cell, king: XY){
            ...
            let newArray: Array<Cell> = [];
                ...
                arrayFigures.push(this.board.getCell(king.y, king.x))
                const dxIt = Math.abs(item!.x - king.x);
                const dyIt = Math.abs(item!.y - king.y);
                if(dxIt === ){
                    arrayFigures.push(item!) 
                }  

                console.log(newArray, arrayFigures);
                
                if(dx === dy && arrayFigures.length === 2){
                    const newA = arrayFigures.filter(el => el.block === true)
                    console.log(newA[0]);
                    return {cell: newA[0], bool: true}
                }
                return {cell: arrayFigures[1], bool: false }
