...
import { Rook } from "./figures/Rook"
    ...
    export class Cell{
        ...
        blackKingKill(cell: Cell, kingParams: XY, king: Cell | null){
            if(cell.figure?.figureName === FigureName.QUEEN){
                if(this.tryToKillKingHorizontal(cell, kingParams, king) || this.tryToKillKingDiagonal(cell, kingParams, king) || this.tryToKillKingVertical(cell, kingParams, king)){
                    return true;
                }
            }
            if(cell.figure?.figureName === FigureName.BISHOP){
                if(this.tryToKillKingDiagonal(cell, kingParams, king)){
                    return true
                }
            }
            if (cell.figure?.figureName === FigureName.KNIGHT) {
                if(this.tryToKillKingWithKnight(cell, kingParams, king)){
                    return true
                }
            }
            if(cell.figure?.figureName === FigureName.PAWN){
                if(this.tryToKillKingWithPawn(cell, kingParams, king)){
                    return true
                }
            }
            if(cell.figure?.figureName === FigureName.ROOK){
                if(this.tryToKillKingHorizontal(cell, kingParams, king) || this.tryToKillKingVertical(cell, kingParams, king)){
                    return true
                }
            }
        }
        
        whiteKingKill(cell: Cell, kingParams: XY, king: Cell | null){
            if(cell.figure?.figureName === FigureName.QUEEN){
                if(this.tryToKillKingHorizontal(cell, kingParams, king) || this.tryToKillKingDiagonal(cell, kingParams, king) || this.tryToKillKingVertical(cell, kingParams, king)){
                    return true;
                }
