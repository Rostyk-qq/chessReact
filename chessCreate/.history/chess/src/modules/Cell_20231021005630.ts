...
import { Rook } from "./figures/Rook"
    ...
    export class Cell{
        ...
        constructor(board: Board, y: number, x: number,color: Color, figure: Figure | null){
            ...
            this.whiteRook = false
            this.blackRook = false

            this.whiteBishop = false
            this.blackBishop = false

            this.whitePawn = false
            this.blackPawn = false

            this.whiteKnight = false
            this.blackKnight = false
        }
        ...
        moveFigure(target: Cell){
            if(this.figure && this.figure?.canMove(target)){
                ...
                if(target.figure?.color === Color.WHITE){
                    if(target.figure.figureName === FigureName.QUEEN){
                        target.whiteQueen = false
                    }
                    if(target.figure.figureName === FigureName.BISHOP){
                        target.whiteBishop = false
                    }
                    if(target.figure.figureName === FigureName.ROOK){
                        target.whiteRook = false
                    }
                    if(target.figure.figureName === FigureName.PAWN){
                        target.whitePawn = false
                    }
        ...
        moveHorizontalKingDisabledWhite(target: Cell){
            ...
            for(let i = min + 1; i < max; i++){
                if(!this.board.getCell(this.y, i).isEmpty()){
                    if(this.board.getCell(this.y, i).figure?.color === Color.WHITE || this.board.getCell(this.y, i).figure?.color === Color.BLACK){
                        return false  
                    }
        ...
        moveVerticalKingDisabledWhite(target: Cell){
            ...
            for (let i = min + 1; i < max; i++) {
                if(!this.board.getCell(i, this.x).isEmpty()){
                    if(this.board.getCell(i, this.x).figure?.color === Color.WHITE || this.board.getCell(i, this.x).figure?.color === Color.BLACK){
                        return false  
                    }
        ...
        moveDiagonalKingDisabledBlack(target: Cell){
            ...
            for (let i = 1; i <= dy; i++) {
                if(!this.board.getCell(this.y + (i * directionY), this.x + (i * directionX)).isEmpty()){
                    ...
                    || this.board.getCell(this.y + (i * directionY), this.x + (i * directionX)).figure?.color === Color.WHITE){
                        return false
                    }
                    if(this.board.getCell(this.y + (i * directionY), this.x + (i * directionX)).figure?.figureName === FigureName.KING){
                        return true
        ...
        moveHorizontalKingDisabledBlack(target: Cell){
            ...
            for(let i = min + 1; i < max; i++){
                if(!this.board.getCell(this.y, i).isEmpty()){
                    if(this.board.getCell(this.y, i).figure?.color === Color.BLACK || this.board.getCell(this.y, i).figure?.color === Color.WHITE){
                        return false
                    }
        ...
        moveVerticalKingDisabledBlack(target: Cell){
            ...
            for (let i = min + 1; i < max; i++) {
                if(!this.board.getCell(i, this.x).isEmpty()){
                    if(this.board.getCell(i, this.x).figure?.color === Color.BLACK || this.board.getCell(i, this.x).figure?.color === Color.WHITE){
                        return false  
                    }
