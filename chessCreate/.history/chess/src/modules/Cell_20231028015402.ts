...
export class Cell{
        ...
        whiteFigureBlockBlackKing(figure: Cell, king: XY){
            if(figure.figure?.figureName === FigureName.QUEEN){
                if(this.blockFigureMovementWhiteKingDiagonal(figure, king) ||
                this.blockFigureMovementWhiteKingVertical(figure, king) ||
                this.blockFigureMovementWhiteKingHorizontal(figure, king)){
                    const result: Result = this.blockFigureMovementWhiteKingDiagonal(figure, king) | this.blockFigureMovementWhiteKingVertical(figure, king);
                    if(!result.booleanValue){
                        result.item!.block = false   
                    }
                    result.item!.block = false
                }
            }
            if(figure.figure?.figureName === FigureName.ROOK){
                if(this.blockFigureMovementWhiteKingVertical(figure, king) ||
                this.blockFigureMovementWhiteKingHorizontal(figure, king)){
                    console.log('ROOK');
                    return true
                }
            }
            if(figure.figure?.figureName === FigureName.BISHOP){
                if(this.blockFigureMovementWhiteKingDiagonal(figure, king)){
                    console.log('BISHOP');
                    return true
                }
        ...
        blockFigureMovementWhiteKingDiagonal(figure: Cell, king: XY) {
            ...
            if (dx !== dy) {
              return {booleanValue: false, item: null};
            }
          
            for (let i = 1; i < dy; i++) {
              const targetCell = this.board.getCell(figure.y + (i * directionY), figure.x + (i * directionX));
              
              if (!targetCell.isEmpty()) {
                if (targetCell.figure?.figureName !== FigureName.KING || targetCell.figure?.color === Color.BLACK) {
                  a = a + 1;
                }
              }
            }
            ...
            if (a === 1) {
               return {booleanValue: true, item: item!}
            }
            return {booleanValue: false, item: item!}
          }
        blockFigureMovementWhiteKingVertical(figure: Cell, king: XY){
            if(figure.x !== king.x){
                return false
            }
            ...
            for (let i = min + 1; i < max; i++) {
                if(!this.board.getCell(i, this.x).isEmpty()){
                    if(this.board.getCell(i, figure.x).figure?.figureName !== FigureName.KING 
                    || this.board.getCell(i, figure.x).figure?.color === Color.BLACK){
                        a = a + 1
        ...
        blockFigureMovementWhiteKingHorizontal(figure: Cell, king: XY){
            if(figure.y !== king.y){
                return false
            }
            ...
            for (let i = min + 1; i < max; i++) {
                if(!this.board.getCell(this.y, i).isEmpty()){
                    if(this.board.getCell(figure.y, i).figure?.figureName !== FigureName.KING 
                    || this.board.getCell(figure.y, i).figure?.color === Color.BLACK){
                        a = a + 1
