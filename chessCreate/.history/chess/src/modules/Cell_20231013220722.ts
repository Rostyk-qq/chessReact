...
interface XY {
    ...
    export class Cell{
        ...
        whiteKingKill(cell: Cell, kingParams: XY){
            const king = new King(Color.BLACK, this.board.getCell(kingParams.y, kingParams.x))
            king.canMove(cell)
        ...
        tryToKillKingDiagonal(cell: Cell, kingParams: XY){        
            ...
            for (let i = 1; i < dx; i++) {
                if(!this.board.getCell(cell.y + (i * directionY), cell.x + (i * directionX)).isEmpty()){
                    this.arrayWithDXDY.push({x: cell.x + (i * directionX), y: cell.y + (i * directionY)})
        ...
        tryToKillKingWithPawn(cell: Cell, kingParams: XY){
            ...
            if(kingParams.y === cell.y + direction && (kingParams.x === cell.x + 1 || kingParams.x === cell.x - 1)){
                return true
            }
            return false
        }
