...
export class Cell{
        ...
        blockFigureMovementDiagonal(figure: Cell, king: XY){
            ...
            const arrayFigures: Array<Cell> = [];
                ...
                for (let i = 0; i < dy; i++) {
                    if(!this.board.getCell(figure.y + (i * directionY) + 1, figure.x + (i * directionX) + 1).isEmpty()){
                        item = this.board.getCell(figure.y + (i * directionY) + 1, figure.x + (i * directionX) + 1) 
                        arrayFigures.push(item)                      
                    }
                }

                if(arrayFigures.length === 1 && dx === dy){
