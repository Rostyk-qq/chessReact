...
export class Cell{
        ...
        moveFigure(target: Cell){
            ...
            DiagonalKill(start: Cell, end: Cell, king: XY, dx: number, dy: number){
                ...
                if(dx !== dy || dxCurrent !== dyCurrent || dx !== dxCurrent || dy !== dyCurrent){
                    return false
                }

                for(let i = 1; i <= dxCurrent; i++){
                    if(this.board.getCell(start.y + (i * directionY), start.x + (i * directionX)).figure === null){
                        return true
                    }
                }
                return false        
