...
import { Rook } from "./figures/Rook"
    ...
    export class Cell{
        ...
        moveDiagonal(target: Cell, king?: Figure){
            ...
            for (let i = 1; i < dy; i++) {
                if(king){
                    if(king.cell.y !== this.y + (i * directionY) && king.cell.x !== this.x + (i * directionX)){
                        return true
                    }
                    return false
                }
        ...
        moveHorizontal(target: Cell, king?: Figure){
            if(target.y !== this.y){
                return false
            }
        ...
        moveVertical(target: Cell, king?: Figure){
            if(target.x !== this.x){
                return false;
            }
        ...
        findFigureDiagonal(cell: Cell, kingParams: XY){        
            const dx = Math.abs(cell.x - kingParams.x);
            const dy = Math.abs(cell.y - kingParams.y);
            ...
            for (let i = 1; i <= dx; i++) {
                if(!this.board.getCell(cell.y + (i * directionY), cell.x + (i * directionX)).isEmpty()){
                    ...
                    if (this.board.getCell(cell.y + (i * directionY), cell.x + (i * directionX)).figure?.figureName !== FigureName.KING) {
                        return this.board.getCell(cell.y + (i * directionY), cell.x + (i * directionX)).figure
                    }
                }
            }
        }

        findFigureHorizontal(cell: Cell, kingParams: XY){  
            if(cell.y !== kingParams.y){
                return
            }    
            ...
            for (let i = min; i < max; i++) {
                if(!this.board.getCell(cell.y, i).isEmpty()){
                    if (this.board.getCell(cell.y, i).figure?.figureName !== FigureName.KING) {
                        return this.board.getCell(cell.y, i).figure
                    }
                }
            }
        }

        findFigureVertical(cell: Cell, kingParams: XY) {
            if(cell.x !== kingParams.x){
                return
            }    
            ...
            for (let i = min + 1; i < max; i++) {
                if(!this.board.getCell(i, cell.x).isEmpty()){
                    if (this.board.getCell(i, cell.x).figure?.figureName !== FigureName.KING) {
                        return this.board.getCell(i, cell.x).figure
                    }
                }
            }
        }
