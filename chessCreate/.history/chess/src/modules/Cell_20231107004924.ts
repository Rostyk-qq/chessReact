...
export class Cell{
        ...
        moveFigure(target: Cell){
            ...
            VerticalKill(start: Cell, end: Cell){
                ...
                for(let i = 1; i <= dY; i++){
                    if(this.board.getCell(start.y + (i * directionY), start.x + (i * directionY)).isEmpty()
                    || this.board.getCell(start.y + (i * directionY), start.x + (i * directionY)).figure?.color !== start.figure?.color){
                        return true
                    }
                }
                return false
