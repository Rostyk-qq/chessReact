...
import { Rook } from "./figures/Rook"
    ...
    export class Cell{
        ...
        clearToCastling(clickedCell: Cell, currentCell: Cell){
            ...
            for (let i = min + 1; i < max; i++) {
                if(!this.board.getCell(currentCell.y, i).isEmpty()){
                    return false
                }
            }
            return true
        ...
        Castling(clicked: Cell, current: Cell, XY: XYCastling){
            ...
            setTimeout(() => {
                new Rook(clicked.figure?.color!, this.board.getCell(clicked.y, clicked.x));
                new King(current.figure?.color!, this.board.getCell(current.y, current.x));
            }, 200)    
