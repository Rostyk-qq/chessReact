...
export class Cell{
        ...
        blockFigureMovementWhiteKingDiagonal(figure: Cell, king: XY){
            ...
            if(arrayFigures.length === 2){
                arrayFigures.map(cells => {
                    if(cells.figure?.figureName !== FigureName.KING){
                        return {cell: cells, bool: true}
                    }
                    else{
                        return {cell: cells, bool: false}
                    }
                })
                // if(a === 2 && item.figure?.figureName === FigureName.KING){
                    // return {cell: item!, bool: true}
                // }
                // else{
                //     return {cell: item!, bool: false}
                // }
