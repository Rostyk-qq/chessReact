...
import { Rook } from "./figures/Rook"
    ...
    export class Cell{
        ...
        BlockingKnightLowerWhite(target: Cell, clicked: Cell){
            if((Math.abs(target.x - clicked.x) === 1 && Math.abs(target.y - clicked.y) === 2) || 
            (Math.abs(target.x - this.cell.x) === 2 && Math.abs(target.y - clicked.y) === 1)){
                target.whiteKnight = true
                return true
            }
            target.whiteKnight = false
        }
        BlockingKnightHiderWhite(target: Cell){
            console.log('hider white');
        }
        BlockingKnightLowerBlack(target: Cell){
            console.log('lower black');
        }
        BlockingKnightHiderBlack(target: Cell){
            console.log('hider black');
        }
    }