...
export class Cell{
        ...
        moveFigure(target: Cell){
            ...
            DiagonalKill(start: Cell, end: Cell){
                    ...
                    const dxL = Math.abs(start.x - this.figure?.cellWhiteAttack.cell.x!);
                    const dyL = Math.abs(start.y - this.figure?.cellWhiteAttack.cell.y!)

                    const directionXL = start.x < this.figure?.cellWhiteAttack.cell.x! ? 1 : -1;    
                    const directionYL = start.y < this.figure?.cellWhiteAttack.cell.y! ? 1 : -1;

                    const directionX = start.x < end.x ? 1 : -1;    
                    const directionY = start.y < end.y ? 1 : -1;

                    if(dx !== dy || !start.block || dxL !== dyL){
                        return false
                    }
                    ...
                    console.log(this.figure?.cellWhiteAttack.cell.x! < start.x && (((dxL * directionXL) < 0 && dyL * directionYL > 0)));
                    
                    if(this.figure?.cellWhiteAttack.cell.x! < start.x && ((dxL * directionXL < 0 && dyL * directionYL > 0))){
                        for(let i = 1; i < dxL; i++){
