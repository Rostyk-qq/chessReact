        ...
        if(cell.figure?.color === Color.WHITE && cell.figure.figureName === FigureName.QUEEN){
            ...
            if(dy !== dx){                
                ...
                for (let i = min; i < max; i++) {
                    if(!this.board.getCell(cell.y, cell.x + (directionX * i)).isEmpty()){
                        if(this.board.getCell(cell.y, cell.x + (directionX * i)).figure?.figureName === FigureName.KING){
                            return true
                        }
                        return false
                    }
            ...
            for (let i = 0; i < dy; i++) {
                if(!this.board.getCell(cell.y + (i * directionY), cell.x + (i * directionX)).isEmpty() && this.board.getCell(cell.y + (i * directionY), cell.x + (i * directionX)).figure?.figureName === FigureName.KING){
                    return true
                }
            }
            return false
