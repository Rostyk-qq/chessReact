    ...
    export class Cell{
        ...
        whiteKingKill(cell: Cell, kingParams: XY){
            ...
            if(cell.figure?.figureName === FigureName.ROOK){
               ...
               const figure: Figure | boolean | undefined = this.findFigureHorizontal(cell, kingParams)
               figure.cell.available = false    
            }
        }
            ...
            for (let i = min + 1; i < max; i++) {
                if(!this.board.getCell(i, cell.x).isEmpty()){
                    if (this.board.getCell(i, cell.x).figure?.figureName !== FigureName.KING) {
                        console.log(this.board.getCell(i, cell.x).available = false);
                        return false
                    }
                }
            }
