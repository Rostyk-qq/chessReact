...
export class Cell{
        ...
        moveFigure(target: Cell){
            ...
            HorizontalKill(end: Cell){
                const directionX = this.x < end.x ? 1 : -1;

                const dx = Math.abs(this.x - end.x);
                const min = Math.min(this.x, end.x)

                for(let i = min; i < dx; i++){
                    if(!this.board.getCell(this.y, this.x  + (i * directionX)).isEmpty()){
                        return false
                    }
                }
                return true
