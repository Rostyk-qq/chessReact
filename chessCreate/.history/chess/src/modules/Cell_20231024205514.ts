...
export class Cell{
        ...
        setDeletedFigures(target: Figure){
            ...
            if(target.figureName === FigureName.PAWN){
                const x1 = this.x + 2
                const x2 = Math.abs(this.x - 2)

                if(target.color === Color.BLACK){
                    this.blackPawn[target.uniqValue!] = false;
                    this.board.getCell(this.y, x1).blackPawn[target.uniqValue!] = false
                    this.board.getCell(this.y, x2).blackPawn[target.uniqValue!] = false
                }
                if(target.color === Color.WHITE){
                    this.whitePawn[target.uniqValue!] = false;
                    this.board.getCell(this.y, x1).whitePawn[target.uniqValue!] = false
                    this.board.getCell(this.y, x2).whitePawn[target.uniqValue!] = false
                }
            }
            if(target.figureName === FigureName.QUEEN){

                if(target.cell.figure?.color === Color.WHITE){
                    if(target.cell.moveDiagonalKingDisabledWhite(target.cell)){
                        target.cell.whiteQueen = false
                        return true
                    }
                    if(target.cell.moveHorizontalKingDisabledWhite(target.cell)){
                        target.cell.whiteQueen = false
                        return true
                    }
                    if(target.cell.moveVerticalKingDisabledWhite(target.cell)){  
                        target.cell.whiteQueen = false
                        return true
                    }
                    target.cell.whiteQueen = false
                ...
                if(target.cell.figure?.color === Color.BLACK){ 
                    if(target.cell.moveDiagonalKingDisabledBlack(target.cell)){
                        target.cell.blackQueen = false
                        return true
                    }
                    if(target.cell.moveHorizontalKingDisabledBlack(target.cell)){
                        target.cell.blackQueen = true
                        return true
                    }
                    if(target.cell.moveVerticalKingDisabledBlack(target.cell)){  
                        target.cell.blackQueen = false
                        return true
                    }
                    target.cell.blackQueen = false
