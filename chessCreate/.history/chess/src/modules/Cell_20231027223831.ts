...
export class Cell{
        ...
        blockFigureMovementWhiteKingDiagonal(figure: Cell, king: XY){
            ...
            const directionX = this.x < figure.x ? 1 : -1;    
            const directionY = this.y < figure.y ? 1 : -1;

            if(dx !== dy){
                return false;
            }
            let figureBlock: Cell, i: number = 0;
            for (let i = 1; i < dy; i++) {
                if(!this.board.getCell(this.y + (i * directionY), this.x + (i * directionX)).isEmpty()){
                    ...
                    || this.board.getCell(this.y + (i * directionY), this.x + (i * directionX)).figure?.color === Color.BLACK){
                        figureBlock = this.board.getCell(this.y + (i * directionY), this.x + (i * directionX))
                        ++i;
                    }
                }
            }
            if(i === 1){
                figureBlock!.block = true
                return true
            }
            return false 
        }
        blockFigureMovementWhiteKingVertical(figure: Cell, king: XY){
            ...
            for (let i = min + 1; i < max; i++) {
                if(!this.board.getCell(i, this.x).isEmpty()){
                    ...
                    || this.board.getCell(i, this.x).figure?.color === Color.BLACK){
                        figureBlock = this.board.getCell(i, this.x)
                        ++i;
                    }
                }
            }
        ...
        blockFigureMovementWhiteKingHorizontal(figure: Cell, king: XY){
            ...
            for (let i = min + 1; i < max; i++) {
                if(!this.board.getCell(this.y, i).isEmpty()){
                    if(this.board.getCell(this.y, this.x).figure?.figureName !== FigureName.KING 
                    || this.board.getCell(this.y, this.x).figure?.color === Color.BLACK){
                        figureBlock = this.board.getCell(i, this.x)
                        ++i;
                    }
                }
