...
import { Rook } from "./figures/Rook"
    ...
    export class Cell{
        ...
        moveDiagonalKingDisabled(target: Cell){
            ...
            for (let i = 1; i < dy; i++) {
                if(!this.board.getCell(this.y + (i * directionY), this.x + (i * directionX)).isEmpty()){
                    if(this.board.getCell(this.y + (i * directionY), this.x + (i * directionX)).figure?.color === ){
        ...
        moveHorizontalKingDisabled(target: Cell){
            if(target.y !== this.y){
                return false
            }
        ...
        moveVerticalKingDisabled(target: Cell){
            if(target.x !== this.x){
                return false;
            }
