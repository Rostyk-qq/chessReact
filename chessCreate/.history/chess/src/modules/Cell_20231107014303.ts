        ...
        moveFigure(target: Cell){
            ...
            DiagonalKill(start: Cell, end: Cell){
                ...
                for(let i = 1; i <= dx; i++){
                    ...
                    (Math.abs(this.board.getCell(start.y + (i * directionY), start.x  + (i * directionX)).y - end.y) === start.x + dy && Math.abs(this.board.getCell(start.y + (i * directionY), start.x  + (i * directionX)).x - end.x) === dx)){
                        return true 
                    }
                }
                return false    
