...
export class Cell{
        ...
        setDeletedFigures(target: Figure){            
            ...
            if(target.figureName === FigureName.QUEEN && target.color === Color.BLACK){
                for(let i = 0; i < target.cell.blackPawn.length; i++){
                    if(target.uniqValue === i){
                        target.cell.blackPawn[i] = false
                    }
                } 
            }
