...
export class Cell{
        ...
        moveFigure(target: Cell){
            ...
            DiagonalKill(start: Cell, end: Cell){
                ...
                if(this.figure?.cellBlackProtect.block && start === this.figure?.cellBlackProtect && end === this.figure?.cellWhiteAttack.cell){
                    ...
                    if(start.x < end.x && ((dx < 0 && dy < 0) || (dx > 0 && dy > 0)) && dx === dy){
                        for(let i = 1; i < Math.abs(dx); i++){
                            if(!this.board.getCell(start.y + i, start.x + i).isEmpty()){
                                return false
                            }
                            return true
                        }
                    ...
                    if(start.x > end.x && ((dx < 0 && dy > 0) || (dx > 0 && dy < 0)) && dx === dy){
                        for(let i = 1; i < Math.abs(dx); i++){
                            if(!this.board.getCell(start.y + (i * -1), start.x + (i * directionX)).isEmpty()){
                                return false
                            }
                        } 
                    }   
