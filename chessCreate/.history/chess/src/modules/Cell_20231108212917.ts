...
export class Cell{
        ...
        moveFigure(target: Cell){
            ...
            DiagonalKill(start: Cell, end: Cell){
                    ...
                    const dxQueen = start.x - this.figure?.cellWhiteAttack.cell.x!;
                    const dyQueen = start.y - this.figure?.cellWhiteAttack.cell.y!;
                    
                    console.log(this.figure?.cellWhiteAttack.cell.x!, this.figure?.cellWhiteAttack.cell.y!);
                    
                    const directionXL = start.x < this.figure?.cellWhiteAttack.cell.x! ? 1 : -1;    
                    const directionYL = start.y < this.figure?.cellWhiteAttack.cell.x! ? 1 : -1;

                    const directionX = start.x < end.x ? 1 : -1;    
                    const directionY = start.y < end.y ? 1 : -1;

                    if(dx !== dy || !start.block){
                        return false
                    }
                    ...
                    console.log(this.figure?.cellWhiteAttack.cell.x! < start.x && ((dx <= 0 && dy >= 0) || (dx >= 0 && dy <= 0)));
                    
                    if(start.x > this.figure?.cellWhiteAttack.cell.x! && ((dxQueen < 0 && dyQueen > 0) || (dxQueen > 0 && dyQueen < 0))){
                        for(let i = 1; i < Math.abs(dxQueen); i++){
                            
                            if(this.board.getCell(start.y + (i * directionYL), start.x + (i * directionXL)).isEmpty()){
                                return true
                            }
