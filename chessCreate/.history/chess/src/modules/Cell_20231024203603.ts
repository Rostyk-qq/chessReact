...
export class Cell{
        ...
        setDeletedFigures(target: Figure){
            ...
            if(target.figureName === FigureName.QUEEN){
                ...
                const whiteQueen = cell?.querySelector('.cell__block_white_queen');
                const blackQueen: Element = cell?.querySelector('.cell__block_black_queen');

                if(target.color === Color.BLACK){
                    cell?.removeChild(blackQueen)
