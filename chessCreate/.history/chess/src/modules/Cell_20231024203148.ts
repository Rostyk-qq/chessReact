...
export class Cell{
        ...
        setDeletedFigures(target: Figure){
            ...
            if(target.figureName === FigureName.QUEEN){
                ...
                const whiteQueen = cell?.querySelector('.cell__block_white .queenWhite');
                const blackQueen = cell?.querySelector('.cell__block_black .queenBlack');

                if(target.color === Color.BLACK){
                    cell?.remove(blackQueen)
