...
export class Cell{
        ...
        blockFigureMovementDiagonal(figure: Cell, king: XY){
            ...
            let arrayFigures: Array<Cell> = [];
                ...
                if(dx !== dy){
                    arrayFigures =  arrayFigures.filter(el => el !== figure)
                }
         
                if(arrayFigures.length === 3 && dx === dy && arrayFigures[1].figure !== null &&
                    (Math.abs(arrayFigures[1].x - arrayFigures[2].x) === Math.abs(arrayFigures[1].x - arrayFigures[2].x))){
                    console.log(arrayFigures);
                    return {cell: arrayFigures[0], bool: true}
                }
                else{
                    console.log(arrayFigures, 'End' );
