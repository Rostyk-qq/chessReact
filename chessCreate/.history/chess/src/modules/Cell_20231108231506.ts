...
export class Cell{
        ...
        moveFigure(target: Cell){
            ...
            DiagonalKill(start: Cell, end: Cell){
                    ...
                    const dxL = start.x - this.figure?.cellWhiteAttack.cell.x!;
                    const dyL = start.y - this.figure?.cellWhiteAttack.cell.y!;

                    const directionX = start.x < end.x ? 1 : -1;    
                    const directionY = start.y < end.y ? 1 : -1;

                    const directionXL = this.figure?.cellWhiteAttack.cell.x! < end.x ? 1 : -1;    
                    const directionYL = this.figure?.cellWhiteAttack.cell.y! < end.y ? 1 : -1;

                    if(Math.abs(dx) !== Math.abs(dx)){
                        return false
                    }
                    if(this.figure!.cell.x > this.figure?.cellWhiteAttack.cell.x! && ((dxL > 0 && dyL < 0) || (dxL > 0 && dyL < 0))){
                        for(let i = 1; i <= Math.abs(dxL); i++){
                    ...
                    if(start.x < this.figure?.cellWhiteAttack.cell.x! && ((dx * directionX < 0 && dy * directionY < 0) || (dx * directionX > 0 && dy  * directionY > 0))){
