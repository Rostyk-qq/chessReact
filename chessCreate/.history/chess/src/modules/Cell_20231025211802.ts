...
export class Cell{
        ...
        setDeletedFigures(target: Figure){
            if(target.cell.figure){
                if(target.color === Color.BLACK){
                    if(target.figureName === FigureName.PAWN){
                        const stepOne = this.figure?.color === Color.BLACK ? 1 : -1
                        const stepTwo = this.figure?.color === Color.BLACK ? 2 : -2

                        let left = this.board.getCell(this.y + , this.x - 2)
                        let right = this.board.getCell(this.y, this.x + 2)

                        this.blackPawn[target.uniqValue!] = false
                        left.blackPawn[target.uniqValue!] = false
                        right.blackPawn[target.uniqValue!] = false
                    }
                    if(target.figureName === FigureName.KNIGHT){
                        const dx = Math.abs(this.x - target.cell.x);
                        const dy = Math.abs(this.y - target.cell.y);

                        if((dy === 1 && dx === 2) || (dy === 2 && dx === 1)){
                            if(target.uniqValue === 3333){
                                target.cell.blackKnightLower = false   
                            }   
                            if(this.figure?.uniqValue === 4444){
