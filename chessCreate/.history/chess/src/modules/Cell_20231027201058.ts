...
export class Cell{
        ...
        blockBlackFigure(cell: Cell, blackKing: XY){
            if(cell.figure?.figureName === FigureName.QUEEN){
                if(this.blockBlackFigureDiagonal(cell, blackKing)){
                    return this.blockBlackFigureDiagonal(cell, blackKing)
                }
            }
        }
        blockBlackFigureDiagonal(cell: Cell, blackKing: XY){
            const dx = Math.abs(cell.x - blackKing.x);
            const dy = Math.abs(cell.y - blackKing.y);

            const directionX = cell.x < blackKing.x ? 1 : -1;    
            const directionY = cell.y < blackKing.y ? 1 : -1;

            if(dx !== dy){
                return;
            }
            let figure = this.board.getCell(this.y + (i * directionY), this.x + (i * directionX))
            for (let i = 1; i <= dy; i++) {
                if(!this.board.getCell(this.y + (i * directionY), this.x + (i * directionX)).isEmpty()){
                    if(this.board.getCell(this.y + (i * directionY), this.x + (i * directionX)).figure?.color === Color.BLACK){
                        return this.board.getCell(this.y + (i * directionY), this.x + (i * directionX))
                    }
                }
            }
        }
        ...
        MovementWhiteDiagonal(cell: Cell){
