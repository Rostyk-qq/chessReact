...
export class Cell{
        ...
        moveFigure(target: Cell){
            ...
            DiagonalKill(start: Cell, end: Cell, king: XY, dx: number, dy: number){
                
                const directionX = king.x < start.x && start.x < end.x ? 1 : -1;
                const directionY = king.y < start.y && start.y < end.y ? 1 : -1;

                const dxCurrent = Math.abs(start.x - end.x);
                const dyCurrent = Math.abs(start.y - end.y);

                console.log(dx, dy);
                
                if(dxCurrent !== dyCurrent && dx !== dxCurrent && dy !== dyCurrent){
                    return false
                }

                for(let i = 0; i < dx; i++){
                    console.log(!this.board.getCell(start.y + (i * directionY), start.x + (i * directionX)).isEmpty() || 
                    this.board.getCell(start.y + (i * directionY), start.x + (i * directionX)).isEmpty());
