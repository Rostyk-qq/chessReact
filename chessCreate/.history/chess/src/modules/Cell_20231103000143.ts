...
export class Cell{
        ...
        blockFigureMovementDiagonal(figure: Cell, king: XY){            
            ...
            blockFigureMovementHorizontal(figure: Cell, king: XY){
                ...
                const max = Math.miminn(figure.x - king.x);    
                const min = Math.abs(figure.x - king.x); 
                ...
                if(figure.x >= 4){
                    for (let i = 1; i < max; i++) {
                        if(!this.board.getCell(figure.y, figure.x + (directionX * i)).isEmpty()){
                            item = this.board.getCell(figure.y, figure.x + (directionX * i))
                            arrayCell.push(item)
                        }
                    }
                }   
                ...
                if(arrayCell.length === 3 && kingCell && figure.y === king.y){
                    this.cellHorizontal = newArray[0]
