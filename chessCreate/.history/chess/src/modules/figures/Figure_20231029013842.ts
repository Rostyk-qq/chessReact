...
export abstract class Figure{
    ...
    if(target.board.arrayWhiteFiguresBlock.length){
        ...
        const dx = Math.abs(target.x - figureMain.king.x);
        const dy = Math.abs(target.y - figureMain.king.y);

        if(cell.cell.figure?.color === Color.BLACK && target.figure?.color === Color.WHITE &&
            (target.x === figureMain.king.x || target.y === figureMain.king.y || dx === dy)){
