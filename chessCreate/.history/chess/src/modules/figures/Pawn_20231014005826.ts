    ...
    public canMove(target: Cell){
        ...
        && target.x === this.cell.x){
            if(target.figure && this.secondStep){
                ...
                return false
                const oneStep = this.cell.y + stepOne;
                const twoStep = this.cell.y + stepTwo;

                let fCell = this.cell.board.getCell(firstCell, target.x);
                let sCell = this.cell.board.getCell(secondCell, target.x);
                
                if(fCell.figure && this.secondStep){
