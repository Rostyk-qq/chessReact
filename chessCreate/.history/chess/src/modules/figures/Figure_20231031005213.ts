    ...
    if(target.board.arrayWhiteFiguresBlock.length){
        this.cell.board.arrayWhiteFiguresBlock.map(figureMain => {
            if(figureMain.figure?.figureName === FigureName.QUEEN && target.figure?.color === Color.WHITE){ 
                ...
                target.blockFigureMovementVertical(figureMain.figure?.cell, {x: target.x, y: this.cell.y}) ||
                target.blockFigureMovementHorizontal(figureMain.figure?.cell, {x: target.x, y: this.cell.y})){
