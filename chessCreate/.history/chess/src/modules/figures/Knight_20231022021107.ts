    ...
    public canBlock(target: Cell){
        ...
        if(this.cell.figure?.figureName === FigureName.KNIGHT){
            if(this.cell.figure?.color === Color.WHITE){
                ...
                else if(target.x === Math.abs(this.cell.x + dx) && target.y === Math.abs(this.cell.y + dy)){
                    target.whiteKnight = false
                }
                else if(this.cell.x === Math.abs(this.cell.x + dx) && this.cell.y === Math.abs(this.cell.y + dy)){
                    target.whiteKnight = false
                }
            }
        }        
