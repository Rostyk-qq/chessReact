    ...
    if(target.board.arrayWhiteFiguresBlock.length){
        ...
        if(this.cell.block && this.cellBlackProtect?.figure?.color === Color.BLACK && this.cellWhiteAttack?.figure?.color === Color.WHITE){
            if(this.cell === this.cellBlackProtect){
                ...
                if(target === this.cellWhiteAttack){
                    const dx = Math.abs(this.cell.x - target.x);
                    const dy = Math.abs(this.cell.y - target.y);

                    const directionX = this.cell.x < target.x ? 1 : -1;
                    const directionY = this.cell.y < target.y ? 1 : -1;
                    ...
                    for (let i = 1; i <= dx; i++) {
                        this.cell.board.getCell(this.cell.y + (i * directionY), this.cell.x + (i * directionX)).available = true
                        if(this.cell.board.getCell(this.cell.y + (i * directionY), this.cell.x + (i * directionX)).available === true){
                            return true
                        }
                    }
                    return true 
