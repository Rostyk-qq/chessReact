...
export abstract class Figure{
    ...
    if(target.board.arrayWhiteFiguresBlock.length){
        ...
        if(this.cell.block && this.cellBlackProtect?.figure?.color === Color.BLACK && this.cellWhiteAttack?.figure?.color === Color.WHITE){
            if(this.cell === this.cellBlackProtect){

                const dx = Math.abs(kingBlack.x - this.cellWhiteAttack.x);
                const dy = Math.abs(kingBlack.y - this.cellWhiteAttack.y);
                
                if(dx === dy && this.cellBlackProtect.block){
                    
                    const directionX = kingBlack.x < this.cellWhiteAttack.x ? 1 : -1;
                    const directionY = kingBlack.y < this.cellWhiteAttack.y ? 1 : -1;


                    for(let i = 1; i <= dx; i++){
                        if(this.cell.board.getCell(kingBlack.y + (i * directionY), kingBlack.x  + (i * directionX)).isEmpty()
                        && this.cell.board.getCell(kingBlack.y + (i * directionY), kingBlack.x  + (i * directionX)).figure?.color !== this.cellBlackProtect.figure?.color){
                            return true 
                        }
                    }
                    return false   
