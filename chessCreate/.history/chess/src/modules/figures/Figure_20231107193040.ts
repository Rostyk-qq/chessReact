    ...
    if(target.board.arrayWhiteFiguresBlock.length){
        ...
        if(this.cell.block && this.cellBlackProtect?.figure?.color === Color.BLACK && this.cellWhiteAttack?.figure?.color === Color.WHITE){
            if(this.cell === this.cellBlackProtect){
                ...
                // const dx = Math.abs(this.cell.x - this.cellWhiteAttack.x)
                // const dy = Math.abs(this.cell.y - this.cellWhiteAttack.y)

                const dx = Math.abs(this.cellBlackProtect.x - this.cellWhiteAttack.x);
                const dy = Math.abs(this.cellBlackProtect.y - this.cellWhiteAttack.y);
                console.log(dx, dy);
                
                if(dx === dy && this.cellBlackProtect.block){
                    if(this.cell.DiagonalKill(this.cell, target, kingBlack)){
                        return true
                    }
                }
                else{
                    this.cell.board.getCell(this.cellBlackProtect.x, this.cellWhiteAttack.y).available = false
                }
