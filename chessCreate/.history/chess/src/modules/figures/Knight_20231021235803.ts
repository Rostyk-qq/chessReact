...
export class Knight extends Figure{
   ...
   public canBlock(target: Cell){
        ...
        const dx = Math.abs(target.x - this.cell.x);
        const dy = Math.abs(target.y - this.cell.y);
        let previusX, previusY;

        if((dx === 1 && dy === 2) || (dx === 2 && dy === 1)){
            
            if(this.cell.figure?.color === Color.WHITE){
                previusX = this.cell.x
                previusY = this.cell.y
            ...
            const dxPrevius = Math.abs(this.cell.x - previusX!);
            const dyPrevius = Math.abs(this.cell.y- previusY!);

            if(dxPrevius === dx && dy === dyPrevius){
