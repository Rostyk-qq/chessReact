        ...
        if(target.board.arrayWhiteFiguresBlock.length){
            this.cell.board.arrayWhiteFiguresBlock.map(figureMain => {
                if(figureMain.figure?.figureName === FigureName.QUEEN){
                    if(target.blockFigureMovementWhiteKingDiagonal(figureMain, king) ||
                    target.blockFigureMovementWhiteKingVertical(figureMain, king) ||
                    target.blockFigureMovementWhiteKingHorizontal(figureMain, king)){
                        console.log('QUEEN');
                        return true
                    }
                }
                if(figure.figure?.figureName === FigureName.ROOK){
                    if(this.blockFigureMovementWhiteKingVertical(figureMain, king) ||
                    this.blockFigureMovementWhiteKingHorizontal(figureMain, king)){
                        console.log('ROOK');
                        return true
                    }
                }
                if(figure.figure?.figureName === FigureName.BISHOP){
                    if(this.blockFigureMovementWhiteKingDiagonal(figureMain, king)){
                        console.log('BISHOP');
                        return true
                    }
                }
