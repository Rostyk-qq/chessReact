...
export abstract class Figure{
    ...
    if(target.board.arrayWhiteFiguresBlock.length){
        ...
        if(this.cell.block){
            this.cell.board.arrayWhiteFiguresBlock.map(cell => {
                const dx = Math.abs(cell.king.x - target.x);
                const dy = Math.abs(cell.king.y - target.y);
                if(this.cell.figure?.color === Color.BLACK && target.figure?.color === Color.WHITE && (cell.king.x === target.x || cell.king.y === target.x || dx === dy)){
                    this.cell.block = false  
                    return true
                }
            })
            if(this.cell.figure?.color === Color.WHITE && target.figure?.color === Color.BLACK && (this.cell.x === target.x || this.cell.x === target.x || dx === dy)){
                this.cell.block = false  
                return true
            }
            return false
