...
export abstract class Figure{
    ...
    if (figureMainObj && cellForKill) {
        ...
        const dxCell = Math.abs(cellForKill.cell.x - figureMainObj.king.x );
        const dyCell = Math.abs(cellForKill.cell.y - figureMainObj.king.y);
        if (
        (cellForKill.cell.figure?.color === Color.BLACK) &&
        target.figure?.color === Color.WHITE && target.figure === figureMainObj.figure!
        (target.x === figureMainObj.king.x || target.y === figureMainObj.king.y || dx === dy)
        && (cellForKill.cell.x === figureMainObj.king.x || cellForKill.cell.y === figureMainObj.king.y || dxCell === dyCell)
        ) {
        return true;
        }
    }
