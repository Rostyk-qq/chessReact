...
export abstract class Figure{
    ...
    if(target.board.arrayWhiteFiguresBlock.length){
        ...
        if(this.cell.block){
            const dx = Math.abs(this.cell.x - king.x);
            const dy = Math.abs(this.cell.y - king.y);
            if(this.cell.figure?.color === Color.BLACK && target.figure?.color === Color.WHITE && (this.cell.x === target.x || this.cell.x === target.x)){
                this.cell.block = false  
                return true
            }
            if(this.cell.figure?.color === Color.WHITE && target.figure?.color === Color.BLACK){
                this.cell.block = false  
                return true
            }
            return false
