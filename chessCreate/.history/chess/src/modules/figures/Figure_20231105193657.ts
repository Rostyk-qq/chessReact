...
export abstract class Figure{
    ...
    if(this.cell.block){
        if (figureMainObj && cellForKill) {
            console.log(cellForKill.cell, figureMainObj.figure?.cell);
            ...
            if (cellForKill.cell.figure?.color === Color.BLACK && figureMainObj.figure) {
                ...
                if(cellForKill.cell.x === figureMainObj.figure.cell.x){
                    ...
                    const max = Math.max(cellForKill.cell.y, figureMainObj.figure.cell.y)
                    const min = Math.max(cellForKill.cell.y, figureMainObj.figure.cell.y)
    
                    const directionY = cellForKill.cell.y < figureMainObj.figure.cell.y ? 1 : -1;
    
                    for(let i = min; i < max; i++){
                        if(!this.cell.board.getCell(min + (i * directionY), cellForKill.cell.x).isEmpty()){
                            return false
                        }
                    }
                    return true
                }
                // if(cellForKill.cell.y === target.y){
                //     if(cellForKill.cell.HorizontalKill(cellForKill.cell, target)){
                //         return true
                //     }
                // }
                // if(dx === dy){
                //     if(cellForKill.cell.DiagonalKill(cellForKill.cell, target)){
                //         return true
                //     }
                // }
                return false
