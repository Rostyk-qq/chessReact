...
export abstract class Figure{
    ...
    public canMove(target: Cell){
        const blackKing: XY = {x: 3, y: 0};
        const whiteKing: XY= {x: 3, y: 7};

        if(this.cell.figure?.figureName === FigureName.KING && target.figure.color === Color.WHITE){
            whiteKing.x = target.x
            whiteKing.y = target.y
        }
        if(this.cell.figure?.figureName === FigureName.KING && this.cell.figure?.color === Color.BLACK){
            blackKing.x = this.cell.x
            blackKing.y = this.cell.y
        }
        console.log(blackKing);
        
        if(target.figure?.color === Color.WHITE){
            if(target.board.arrayWhiteFiguresBlock.length){
                console.log('dfghjklkjhgf');
                
                this.cell.board.arrayWhiteFiguresBlock.map(figureMain => {
                    if(figureMain?.figureName === FigureName.QUEEN){
                        if(this.cell.blockFigureMovementWhiteKingDiagonal(figureMain.cell, blackKing) ||
                        this.cell.blockFigureMovementWhiteKingVertical(figureMain.cell, blackKing) ||
                        this.cell.blockFigureMovementWhiteKingHorizontal(figureMain.cell, blackKing)){
                            this.cell.blackQueen = false
                            return true
                        }
                    }
                    if(figureMain?.figureName === FigureName.BISHOP){
                        if(target.blockFigureMovementWhiteKingDiagonal(figureMain.cell, blackKing) ){
                    ...
                    if(figureMain?.figureName === FigureName.ROOK){
                        if(target.blockFigureMovementWhiteKingVertical(figureMain.cell, blackKing) ||
                        target.blockFigureMovementWhiteKingHorizontal(figureMain.cell, blackKing)){
