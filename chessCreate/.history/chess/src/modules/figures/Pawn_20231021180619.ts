...
export class Pawn extends Figure{
    ...
    public canMove(target: Cell){
        ...
        if((target.y === this.cell.y + stepOne || this.secondStep && (&& (target.y === this.cell.y + stepOne || target.y === this.cell.y + stepTwo) && first))
   ...
   public canBlock(target: Cell){
    ...
    if(this.cell.figure?.color === Color.WHITE && (target.y === this.cell.y + stepOne || target.y === this.cell.y + stepTwo)){
        const stepOne = this.cell.figure?.color === Color.WHITE ? -1 : 1

        if(target.y === this.cell.y + stepOne && !target.figure && (target.x === this.cell.x + 1 || target.x === this.cell.x - 1)){
            target.whitePawn = true
            return true
        }
        target.whitePawn = false
    }
    if(this.cell.figure?.color === Color.BLACK && (target.y === this.cell.y + stepOne || target.y === this.cell.y + stepTwo)){
        const stepOne = this.cell.figure?.color === Color.BLACK ? 1 : -1
