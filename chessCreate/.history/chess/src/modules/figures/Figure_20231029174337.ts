...
export abstract class Figure{
    ...
    let cellForKill: Result = {cell: target, attacker: target, bool: false};
    ...
    if (figureMainObj && cellForKill) {
        const dx = Math.abs(cellForKill.attacker.x - figureMainObj.king.x);
        const dy = Math.abs(cellForKill.attacker.y - figureMainObj.king.y);

        if (
        (cellForKill.cell.figure?.color === Color.BLACK) &&
        target.figure?.color === Color.WHITE &&
        (cellForKill.attacker.x === figureMainObj.king.x || cellForKill.attacker.y === figureMainObj.king.y || dx === dy)
        ) {
        return true;
        }
    }
