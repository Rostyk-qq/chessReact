...
export class Knight extends Figure{
   ...
   public canBlock(target: Cell){
    ...
    const dy = Math.abs(target.y - this.cell.y);
        ...
        if(this.cell.figure?.color === Color.WHITE && this.cell.figure?.figureName === FigureName.KNIGHT){
            if((dx === 1 && dy === 2) || (dx === 2 && dy === 1)){
                target.whiteKnight = true
                return true
            }
            console.log(this.cell.whiteKnight);
        }
        if(this.cell.figure?.color === Color.BLACK && this.cell.figure?.figureName === FigureName.KNIGHT){
            if((dx === 1 && dy === 2) || (dx === 2 && dy === 1)){
