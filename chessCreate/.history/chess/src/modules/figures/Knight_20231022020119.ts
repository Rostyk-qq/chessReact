...
export class Knight extends Figure{
    ...
    public canBlock(target: Cell){
        const dx = Math.abs(target.x - this.normalCell!.x);
        const dy = Math.abs(target.y - this.normalCell!.y);


        if(this.cell.figure?.figureName === FigureName.KNIGHT){
            if((this.cell.figure?.color === Color.WHITE && (this.normalCell?.x === 1 && this.normalCell?.y === 7))){
            ...
            if((this.cell.figure?.color === Color.WHITE  && (this.normalCell?.x === 6 && this.normalCell?.y === 7))){
                if((dx === 1 && dy === 2) || (dx === 2 && dy === 1)){
                    target.whiteKnight = true
                }
                else{
