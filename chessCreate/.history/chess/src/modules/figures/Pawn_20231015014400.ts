...
export class Pawn extends Figure{
    ...
    public canMove(target: Cell){
        ...
        && (target.x === this.cell.x)){
            console.log(target.y, this.cell.y);
            let sCell = this.cell.board.getCell(this.cell.y + stepTwo, target.x);
            if(target.figure && this.secondStep){
                for (let i = this.cell.y; i <= target.y; i++) {
                    if(!this.cell.board.getCell(this.cell.y + (i*stepOne), this.cell.x).isEmpty()){
                        console.log('error');
