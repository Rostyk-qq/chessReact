...
export abstract class Figure{
    ...
    public canMove(target: Cell){
        if(this.cell.figure?.color === Color.BLACK && target.figure?.color === Color.WHITE){
                ...
                for (let i = 1; i <= dy; i++) {
                    if(!this.cell.board.getCell(this.cell.y + (i * directionY), this.x + (i * directionX)).isEmpty()){
                        if(this.cell.board.getCell(this.cell.y + (i * directionY), this.cell.x + (i * directionX)).figure?.figureName !== FigureName.KING 
                        || this.cell.board.getCell(this.cell.y + (i * directionY), this.x + (i * directionX)).figure?.color !== Color.BLACK){
                            return false
                        }
                    }
                }
