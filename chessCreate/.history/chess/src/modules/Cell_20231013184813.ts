...
export class Cell{
    ...
    moveVertical(target: Cell){
        ...
        tryToKillKingWithKnight(cell: Cell, kingParams: XY){ 
            const dx = Math.abs(cell.x - kingParams.x);
            const dy = Math.abs(cell.x - kingParams.x);

            return (kingParams.x === 1 && kingParams.y === 2) || (kingParams.x === 2 && kingParams.y === 1)
        }

        tryToKillKingWithPawn(cell: Cell, kingParams: XY){
            if(kingParams.y === cell.y + 1 && (kingParams.x === cell.x + 1 || kingParams.x === cell.x - 1)){
                return true
            }
            return false
        }
