...
export class Board {
    ...
    doRefreshFigures(targetClicked: Cell | null, target: Cell | null){
        for (let i = 0; i < this.cells.length; i++) {
            const row:Cell[] = this.cells[i];
            for (let j = 0; j < row.length; j++) {
                let cell = row[j];
                cell.available = !!targetClicked?.figure?.canMove(cell)   
                cell.available = !!target?.figure?.canBlock(cell)   
            }
        }
    }

    doRefreshFiguresCanBlok(targetClicked: Cell | null, target: Cell | null){
        for (let i = 0; i < this.cells.length; i++) {
            const row:Cell[] = this.cells[i];
            for (let j = 0; j < row.length; j++) {
                let cell = row[j];
