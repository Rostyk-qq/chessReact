...
import { Rook } from "./figures/Rook"
    ...
    export class Cell{
        ...
        Castling(clicked: Cell, current: Cell, XY: XYCastling){
            new Rook(Color.WHITE.color!, this.board.getCell(clicked.y, XY.rookX));
            new King(Color.WHITE.color!, this.board.getCell(current.y, XY.kingX));
            return true
        }
        ...
        biggerCastling(clicked: Cell, current: Cell, XY: XYCastling) {
            new Rook(clicked.figure?.color!, this.board.getCell(clicked.y, clicked.x)).cell.figure = null;
            new King(current.figure?.color!, this.board.getCell(current.y, current.x)).cell.figure = null;

            setTimeout(() => {
                new Rook(clicked.figure?.color!, this.board.getCell(clicked.y, XY.rookX));
                new King(current.figure?.color!, this.board.getCell(current.y, XY.kingX));
            }, 200)   
        }
    }