...
export class Board {
    ...
    setPawns(){
        for (let i = 0; i < 8; i++) {
            new Pawn(Color.WHITE, this.getCell(6, i), null)
            new Pawn(Color.BLACK, this.getCell(1, i), null)
        }
    }
    setQueen(){
    ...
    setBishop(){
        new Bishop(Color.WHITE, this.getCell(7, 2), null)
        new Bishop(Color.WHITE, this.getCell(7, 5), null)

        new Bishop(Color.BLACK, this.getCell(0, 2), null)
        new Bishop(Color.BLACK, this.getCell(0, 5), null)

        this.arrayWhiteForChange.push(new Bishop(Color.WHITE, this.getCell(7, 2), null))
        this.arrayBlackForChange.push(new Bishop(Color.BLACK, this.getCell(0, 2), null))
    }
    setKnight(){
    ...
    setRook(){
        new Rook(Color.WHITE, this.getCell(7, 0), null)
        new Rook(Color.WHITE, this.getCell(7, 7), null)

        new Rook(Color.BLACK, this.getCell(0, 0), null)
        new Rook(Color.BLACK, this.getCell(0, 7), null)

        this.arrayWhiteForChange.push(new Rook(Color.WHITE, this.getCell(7, 7), null))
        this.arrayBlackForChange.push(new Rook(Color.BLACK, this.getCell(0, 0), null))
    }
    initFigures(){
        this.setPawns()
        this.setQueen()
