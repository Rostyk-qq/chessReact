...
export class Cell{
        ...
        moveFigure(target: Cell){
            ...
            DiagonalKill(start: Cell, end: Cell, blackKing: XY){

                if(this.figure?.cellBlackProtect.block && start === this.figure?.cellBlackProtect){
                    ...
                    const dxKing = Math.abs(blackKing.x - this.figure?.cellWhiteAttack.cell.x);
                    const dyKing = Math.abs(blackKing.y - this.figure?.cellWhiteAttack.cell.y);

                    if(dxKing !== dyKing){
                        return false
                    }

                        for(let i = 1; i <= dyKing; i++){
                            if(!this.board.getCell(start.y + (i * directionY), start.x + (i * directionX))
                            && this.board.getCell(start.y + (i * directionY), start.x + (i * directionX)).figure?.figureName !== FigureName.QUEEN
                            && this.board.getCell(start.y + (i * directionY), start.x + (i * directionX)).figure?.color !== Color.WHITE){
                                return false
                            }
                        }
                        return true 
