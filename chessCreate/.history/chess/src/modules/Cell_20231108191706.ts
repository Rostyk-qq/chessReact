...
export class Cell{
        ...
        moveFigure(target: Cell){
            ...
            DiagonalKill(start: Cell, end: Cell){
                    if(end.x !== this.figure?.cellWhiteAttack.cell){

                    }
                    const dx = start.x - this.figure?.cellWhiteAttack.cell.x!;
                    const dy = start.y - this.figure?.cellWhiteAttack.cell.y!;

                    const directionX = start.x < this.figure?.cellWhiteAttack.cell.x! ? 1 : -1;    
                    const directionY = start.y < this.figure?.cellWhiteAttack.cell.y! ? 1 : -1;
                    ...
                    console.log(this.figure?.cellWhiteAttack.cell.x);
                    
                    if(start.x > this.figure?.cellWhiteAttack.cell.x! && ((dx < 0 && dy > 0) || (dx > 0 && dy < 0))){
                        for(let i = 1; i <= Math.abs(dy); i++){
                            console.log(this.board.getCell(start.y + (i * directionY), start.x + (i * directionX)));
