...
export class Board {
    ...
    doRefreshFiguresCanBlokWhite(target: Cell | null){
        for (let i = 0; i < this.cells.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
                let cell = row[j];
                if(!!target?.figure?.canBlockWhite(cell)){
                    cell.block = !!target?.figure?.canBlockWhite(cell)! 
                }
            }
        }
    }
    doRefreshFiguresCanBlokBlack(target: Cell | null){
        for (let i = 0; i < this.cells.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
                let cell = row[j];
                if(!!target?.figure?.canBlockBlack(cell)){  
                    cell.block = !!target?.figure?.canBlockBlack(cell)! 
                }
            }
        }
    }
