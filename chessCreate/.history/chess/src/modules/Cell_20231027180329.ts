...
export class Cell{
        ...
        forBlockWhiteFiguresBlackAttack(target: Cell){
            const dx = Math.abs(target.x - this.x)
            const dy = Math.abs(target.y - this.y)

            const directionX = target.x < this.x ? -1 : 1
            const directionY = target.x < this.x ? -1 : 1

            if(dx !== dy){
                return false
            }
            let firstFigure, kingWhite;
            for (let i = 1; i <= dx; i++) {
                if(!this.board.getCell(this.y + (i + directionY), this.x + (i + directionX)).isEmpty()){
                    ...
                    && this.board.getCell(this.y + (i + directionY), this.x + (i + directionX)).figure?.color === Color.WHITE){
                        firstFigure = this.board.getCell(this.y + (i + directionY), this.x + (i + directionX))
                    }
                }
                if(this.board.getCell(this.y + (i + directionY), this.x + (i + directionX)).figure?.figureName === FigureName.KING
                && this.board.getCell(this.y + (i + directionY), this.x + (i + directionX)).figure?.color === Color.WHITE){
                    kingWhite = this.board.getCell(this.y + (i + directionY), this.x + (i + directionX))
                }

                if(kingWhite && firstFigure){
                    return true
                }
            }
            return false    
