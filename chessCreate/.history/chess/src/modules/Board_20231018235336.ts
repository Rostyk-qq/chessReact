...
export class Board {
    ...
    doRefreshFiguresCanBlokWhite(target: Cell | null){
        for (let i = 0; i < this.cells.length; i++) {
            const row:Cell[] = this.cells[i];
            for (let j = 0; j < row.length; j++) {
                let cell = row[j];
                if(!!target?.figure?.canBlock(cell) && cell.figure?.color === Color.WHITE){
                    cell.block = !!target?.figure?.canBlock(cell)! 
                }
            }
        }
    }
    doRefreshFiguresCanBlokBlack(target: Cell | null){
        for (let i = 0; i < this.cells.length; i++) {
            const row:Cell[] = this.cells[i];
            for (let j = 0; j < row.length; j++) {
                let cell = row[j];
                if(!!target?.figure?.canBlock(cell) && cell.figure?.color === Color.BLACK){
                    console.log('black');
