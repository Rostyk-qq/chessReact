        ...
        for (let i = 0; i < this.cells.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
                ...
                if(!!target?.figure?.canBlock(cell) && target.figure.color === Color.WHITE){
                    if(target.figure.figureName === FigureName.QUEEN && target.x === x && target.y === y){
                        target.whiteQueen = !!target?.figure?.canBlock(cell)!
                    }
                    if(target.figure.figureName === FigureName.BISHOP && target.x === x && target.y === y){
                        target.whiteBishop = !!target?.figure?.canBlock(cell)!
                    }
                    if(target.figure.figureName === FigureName.ROOK && target.x === x && target.y === y){
                        target.whiteRook = !!target?.figure?.canBlock(cell)!
                    }
                    if(target.figure.figureName === FigureName.PAWN && target.x === x && target.y === y){
