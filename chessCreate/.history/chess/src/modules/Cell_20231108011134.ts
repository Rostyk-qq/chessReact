...
export class Cell{
        ...
        moveFigure(target: Cell){
            ...
            DiagonalKill(start: Cell, end: Cell){
                ...
                const directionX = start.x < figure.x ? -1 : 1;    
                const directionY = start.y < figure.y ? -1 : 1;

                const dx = start.x - end.x;
                const dy = start.y - end.y;
                
                if(start.x < end.x && ((dx <= 0 && dy <= 0 && dx === dy) || (dx >= 0 && dy >= 0 && dx === dy))){
                ...
                for(let i = 1; i <= dx; i++){
                    if(!this.board.getCell(start.y + (i * directionY), start.x + (i * directionX)).isEmpty()){
                        if(this.board.getCell(start.y + (i * directionY), start.x + (i * directionX)).figure?.figureName === end.figure?.figureName
                        && this.board.getCell(start.y + (i * directionY), start.x + (i * directionX)).figure?.color === end.figure?.color)
                        return true
                    }
                }
                return false        
