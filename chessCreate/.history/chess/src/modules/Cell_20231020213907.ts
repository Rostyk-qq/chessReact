...
import { Rook } from "./figures/Rook"
    ...
    export class Cell{
        ...
        constructor(board: Board, y: number, x: number,color: Color, figure: Figure | null){
            ...
            this.id = Math.random()
            this.whiteBlock = false
            this.blackBlock = false
        }
        ...
        moveHorizontalKingDisabledWhite(target: Cell){
            ...
            for(let i = min + 1; i < max; i++){
                if(!this.board.getCell(this.y, i).isEmpty()){
                    ...
                    if(this.board.getCell(this.y, i).figure?.figureName === FigureName.KING || this.board.getCell(i, this.x).figure?.color === Color.BLACK){
                        return false  
                    }
                }
            }
