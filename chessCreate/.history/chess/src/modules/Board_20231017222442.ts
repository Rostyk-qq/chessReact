    ...
    doRefreshFigures(targetClicked: Cell | null){
        for (let i = 0; i < this.cells.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
                ...
                if(!!targetClicked?.figure?.canMove(cell) && targetClicked.figure?.figureName === FigureName.QUEEN){
                    cell.block = !!targetClicked?.figure?.canMove(cell) 
                    if(targetClicked !== cell){
                        !!targetClicked?.figure?.canMove(cell) = false
                    }
                }            
