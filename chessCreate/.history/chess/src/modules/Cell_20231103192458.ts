...
export class Cell{
        ...
        blockFigureMovementDiagonal(figure: Cell, king: XY){            
            ...
            if(dx !== dy){
                ...
                for (let i = 1; i <= dy; i++) {                            if(!this.board.getCell(figure.y + (i * directionY), figure.x + (i * directionX)).isEmpty()){            arrayFigures.push(this.board.getCell(figure.y + (i * directionY), figure.x + (i * directionX)))                           }    }
                arrayFigures.push(figure)
                if(arrayFigures.length === 3 && dx === dy &&         ((figure.figure?.figureName === FigureName.QUEEN && figure.figure?.color === Color.WHITE) ||         (figure.figure?.figureName === FigureName.ROOK && figure.figure?.color === Color.WHITE))){
                    this.cellDiagonal = arrayFigures[0]        return {cell: arrayFigures[0], bool: true}    }    else{        const newA = arrayFigures.filter(el => el.block === true)        return {cell: newA[0], bool: false}    }
            }
            ...
            blockFigureMovementHorizontal(figure: Cell, king: XY){
                ...
                if(arrayCell.length === 3 && figure.y === king.y && 
                    ((figure.figure?.figureName === FigureName.QUEEN && figure.figure?.color === Color.WHITE) || 
                    (figure.figure?.figureName === FigureName.ROOK && figure.figure?.color === Color.WHITE))){
