...
const CellComponent:FC<CellProps> = ({cell, click, selected}) => {
    return (
        <div onClick={() => click(cell)} style={{backgroundColor: cell.available && cell.figure?.figureName !== FigureName.KING && cell.figure ? 'green' : ''}} className={['cell', cell.color, selected && cell.figure && !cell.block ? 'selected' : ''].join(' ')}>
            {cell.available && !cell.block && !cell.figure && <div className='available'></div>}
