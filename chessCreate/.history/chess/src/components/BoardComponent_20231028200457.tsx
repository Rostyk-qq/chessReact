...
const BoardComponent: FC<BoardProps> = ({
  ...
  const [kingWhiteCell, setKingWhiteCell] = useState<Cell | null>(null);
  const [kingBlackCell, setKingBlackCell] = useState<Cell | null>(null);
  ...
  const click = (cell: Cell) => {
    if (clicked && cell !== clicked && clicked.figure?.canMove(cell)) {
      if((clicked.figure?.figureName === FigureName.QUEEN 
        || cell.figure?.figureName === FigureName.BISHOP
        || cell.figure?.figureName === FigureName.ROOK) 
        && clicked.figure?.color === Color.WHITE){
              clicked.board.arrayWhiteFiguresBlock.push({figure: clicked.figure, king: whiteKing})
        }
        ...
        && clicked.figure?.color === Color.BLACK){
              clicked.board.arrayBlackFiguresBlock.push({figure: clicked.figure, king: blackKing})
        }   
