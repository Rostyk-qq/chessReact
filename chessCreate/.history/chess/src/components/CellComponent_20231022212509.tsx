...
const CellComponent:FC<CellProps> = ({cell, click, selected, clicked}) => {
    return (
        <div onClick={() => click(cell)} style={{backgroundColor: cell.available && cell.figure?.figureName !== FigureName.KING && cell.figure ? 'green' : ''}} className={['cell', cell.color, selected && cell.figure ? 'selected' : ''].join(' ')}>
            ...
            {cell.blackKnight && cell.figure?.color !== Color.BLACK && clicked?.figure?.uniqValue === 3333 && <div className='cell__block_black'></div>}
            {cell.blackKnight && cell.figure?.color !== Color.BLACK && clicked?.figure?.uniqValue === 4444 && <div className='cell__block_black'></div>}
            {cell.whiteKnight && cell.figure?.color !== Color.WHITE && clicked?.figure?.uniqValue === 1111 &&  <div className='cell__block_white'></div>}
            {cell.whiteKnight && cell.figure?.color !== Color.WHITE && clicked.figure?.uniqValue === 2222 &&  <div className='cell__block_white'></div>}
