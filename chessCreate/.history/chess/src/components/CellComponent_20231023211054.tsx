...
const CellComponent:FC<CellProps> = ({cell, click, selected, clicked}) => {
    return (
        <div onClick={() => click(cell)} style={{backgroundColor: cell.available && cell.figure?.figureName !== FigureName.KING && cell.figure ? 'green' : ''}} className={['cell', cell.color, selected && cell.figure ? 'selected' : ''].join(' ')}>
            ...
            {cell.Pawn5 && cell.figure?.color !== Color.BLACK && <div className='cell__block_black'></div>&& cell.figure?.color !== Color.WHITE && clicked?.figure?.color === Color.WHITE ? <div className='cell__block_white'></div> : <div className='cell__block_black'></div>}
            {cell.Pawn6 && cell.figure?.color !== Color.BLACK && <div className='cell__block_black'></div>}
