...
const ChangeFigures:FC<ChangeFiguresProp> = ({array, pawn, setPawn, board, king, color}) => {
    function getFigure(figure: Figure){
        ...
        setPawn(pawn);
        if(pawn.figureName === FigureName.KING){
            pawn.canBlock(pawn.cell)
        }
        if(pawn.figureName === FigureName.QUEEN){
            pawn.canBlock(pawn.cell)
        }
        if(pawn.figureName === FigureName.BISHOP){
            pawn.canBlock(pawn.cell)
        }
        if(pawn.figureName === FigureName.ROOK){
            pawn.canBlock(pawn.cell)
        }
        setTimeout(() => {
