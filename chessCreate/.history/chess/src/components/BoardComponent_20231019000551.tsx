...
const BoardComponent:FC<BoardProps> = ({board, setBoard, change, currentColor}) => {
    ...
    const [availableWhiteCasting, setAvailableWhiteCasting] = useState<boolean>(f
        \])
    ...
    const refresh = () => {
         ...
         if(currentColor?.color === Color.WHITE){
            board.doRefreshFiguresCanBlokWhite(cell)
         }
         if(currentColor?.color === Color.BLACK){
            board.doRefreshFiguresCanBlokBlack(cell)
         }
         createNewInstance()
    } 
