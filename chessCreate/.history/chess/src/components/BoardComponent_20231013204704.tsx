...
const BoardComponent:FC<BoardProps> = ({board, setBoard, change, currentColor}) => {
    ...
    const click = (cell: Cell) => {
        if(clicked && cell !== clicked && clicked.figure?.canMove(cell)){
            ...
            setTimeout(() => {
                if(cell.figure?.color === Color.WHITE || cell.figure?.color === Color.BLACK ){
                    if(cell.figure.figureName === FigureName.QUEEN){
                        if(cell.tryToKillKingDiagonal(cell, cell.figure?.color === Color.BLACK ? whiteKing : blackKing) || cell.tryToKillKingHorizontal(cell, blackKing) || cell.tryToKillKingVertical(cell, blackKing)){
                            cell.figure?.color === Color.BLACK ? alertWhiteKing() : alertBlackKing()
                        }
                    }
                    if(cell.figure.figureName === FigureName.ROOK){
                        if(cell.tryToKillKingHorizontal(cell, cell.figure?.color === Color.BLACK ? whiteKing : blackKing) || cell.tryToKillKingVertical(cell, blackKing)){
                            cell.figure?.color === Color.BLACK ? alertWhiteKing() : alertBlackKing()
                        }
                    }
                    if(cell.figure.figureName === FigureName.BISHOP){
                        if(cell.tryToKillKingDiagonal(cell, cell.figure?.color === Color.BLACK ? whiteKing : blackKing)){
                            cell.figure?.color === Color.BLACK ? alertWhiteKing() : alertBlackKing()
                        }
                    }
                    if(cell.figure.figureName === FigureName.KNIGHT){
                        if(cell.tryToKillKingWithKnight(cell, cell.figure?.color === Color.BLACK ? whiteKing : blackKing)){
                            cell.figure?.color === Color.BLACK ? alertWhiteKing() : alertBlackKing()
                        }
                    }
