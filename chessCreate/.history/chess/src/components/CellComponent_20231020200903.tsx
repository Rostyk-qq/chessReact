...
const CellComponent:FC<CellProps> = ({clickedCell, cell, click, selected}) => {
    return (
        <div onClick={() => click(cell)} style={{backgroundColor: cell.available && cell.figure?.figureName !== FigureName.KING && cell.figure ? 'green' : ''}} className={['cell', cell.color, selected && cell.figure ? 'selected' : ''].join(' ')}>
            ...
            {cell.blackBlock && cell.figure?.color === Color.BLACK && <div className='cell__block_white'></div>}
            {cell.whiteBlock && cell.figure?.color === Color.WHITE && <div className='cell__block_black'></div>}
