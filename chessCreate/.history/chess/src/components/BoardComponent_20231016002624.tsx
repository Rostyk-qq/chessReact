...
const BoardComponent:FC<BoardProps> = ({board, setBoard, change, currentColor}) => {
    ...
    const [blackKingDX, setBlackKingDX] = useState<number>(0);
    const [blackKingDY, setBlackKingDY] = useState<number>(0);

    const [whiteKingDX, setWhiteKingDX] = useState<number>(0);
    const [whiteKingDY, setWhiteKingDY] = useState<number>(0);
    ...
    const [whiteKingForMove, setWhiteKingForMove] = useState<Cell | null>(null)
    const [blackKingForMove, setBlackKingForMove] = useState<Cell | null>(null)
    ...
    const click = (cell: Cell) => {
        if(cell.figure?.figureName === FigureName.KING && cell !== clicked){
            if(cell.figure.color === Color.BLACK){
                ...
                setBlackKingForMove(cell)
                setBlackKingDX(Math.abs(clicked!.x - cell.x))
                setBlackKingDY(Math.abs(clicked!.y - cell.y))
            }
            else{
                setWhiteKing({...whiteKing, x: cell.x, y: cell.y})
                setWhiteKingForMove(cell)
                setWhiteKingDX(Math.abs(clicked!.x - cell.x))
                setWhiteKingDY(Math.abs(clicked!.y - cell.y))
            }
        }
        ...
        if(clicked && cell !== clicked && cell.figure?.figureName !== FigureName.KING && clicked.figure?.canMove(cell)){
            ...
            setTimeout(() => {
                if(cell.figure?.color === Color.WHITE){
                    if(cell.blackKingKill(cell, blackKing, blackKingForMove, {x: blackKingDX, y: blackKingDY})){
                        alertBlackKing()
                    }
                }
                if(cell.figure?.color === Color.BLACK){
                    if(cell.whiteKingKill(cell, whiteKing, whiteKingForMove, {x: whiteKingDX, y: blackKingDY})){
                        alertWhiteKing()
                    }
                }
            }, 100)
