...
const BoardComponent:FC<BoardProps> = ({board, setBoard, change, currentColor}) => {
    ...
    const click = (cell: Cell) => {
        if(clicked && cell !== clicked && clicked.figure?.canMove(cell)){
            ...
            if((clicked.figure.figureName === FigureName.ROOK && clicked.figure.color === Color.WHITE) 
            && (cell.figure?.figureName === FigureName.KING && cell.figure.color === Color.WHITE)){
                if((clicked.figure.cell.y === 7 && clicked.figure.cell.x === 0) 
                && (cell.figure.cell.y === 7 && cell.figure.cell.x === 3)){
                    if(clicked.clearToCastling(clicked, cell)){
                        setTimeout(() => {
                            cell.Castling(clicked.figure, cell.figure)  
                         })  
                    }
                }
                ...
                && (cell.figure.cell.y === 7 && cell.figure.cell.x === 3)){
                    if(clicked.clearToCastling(clicked, cell)){
                        setTimeout(() => {
                            cell.biggerCastling(clicked.figure | null, cell.figure)  
                         })  
                    } 
                }
            }
