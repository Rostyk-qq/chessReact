...
const CellComponent:FC<CellProps> = ({cell, click, selected}) => {
    return (
        <div onClick={() => click(cell)} style={{backgroundColor: cell.available && cell.figure?.figureName !== FigureName.KING && cell.figure ? 'green' : ''}} className={['cell', cell.color, selected && cell.figure ? 'selected' : ''].join(' ')}>
            ...
            {cell.whitePawn8 && cell.figure?.color !== Color.WHITE && <div className='cell__block_white'></div>}

            {cell.blackPawn1 && cell.figure?.color !== Color.BLACK && <div className='cell__block_black'></div>}
            {cell.blackPawn2 && cell.figure?.color !== Color.BLACK && <div className='cell__block_black'></div>}
            {cell.blackPawn3 && cell.figure?.color !== Color.BLACK && <div className='cell__block_black'></div>}
            {cell.blackPawn4 && cell.figure?.color !== Color.BLACK && <div className='cell__block_black'></div>}
            {cell.blackPawn5 && cell.figure?.color !== Color.BLACK && <div className='cell__block_black'></div>}
            {cell.blackPawn6 && cell.figure?.color !== Color.BLACK && <div className='cell__block_black'></div>}
            {cell.blackPawn7 && cell.figure?.color !== Color.BLACK && <div className='cell__block_black'></div>}
            {cell.blackPawn8 && cell.figure?.color !== Color.BLACK && <div className='cell__block_black'></div>}



            {/* {
                cell.whitePawn.map(pawnWhite => (
                    pawnWhite === true && <div className='cell__block_white'></div> 
                ))}
                {cell.blackPawn.map(pawnBlack => (
                    pawnBlack === true && <div className='cell__block_black'></div> 
                ))
            } */}
        </div>
    )
