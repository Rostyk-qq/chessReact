...
const BoardComponent:FC<BoardProps> = ({board, setBoard, change, currentColor}) => {
    ...
    const [blackKingDX, setBlackKingDX] = useState<XY>({y: 0 , x: 0});
    const [blackKingDY, setBlackKingDY] = useState<XY>({y: 0 , x: 0});

    const [whiteKingDX, setWhiteKingDX] = useState<XY>({y: 7 , x: 3});
    const [whiteKingDY, setWhiteKingDY] = useState<XY>({y: 007 , x: 3});
