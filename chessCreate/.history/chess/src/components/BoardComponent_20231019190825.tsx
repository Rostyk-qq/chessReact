    ...
    const click = (cell: Cell) => {
        if(clicked && cell !== clicked && clicked.figure?.canMove(cell)){ 
            ...
            if(availableWhiteCasting){   
                if((clicked.y === 7 && clicked.x === 0) && (cell.y === 7 && cell.x === 3) && cell.figure){
                    console.log('12333311');  
                ...
                if((clicked.y === 7 && clicked.x === 7) && (cell.y === 7 && cell.x === 3) && cell.figure){
                    if(clicked.clearToCastling(clicked, cell)){
            ...
            if(availableBlackCasting){
                if((clicked.y === 0 && clicked.x === 0) && (cell.y === 0 && cell.x === 3) && cell.figure){
                    if(clicked.clearToCastling(clicked, cell)){
                        const XY = {kingX: 1, rookX: 2}
                ...
                if((clicked.y === 0 && clicked.x === 7) && (cell.y === 0 && cell.x === 3) && cell.figure){
                    if(clicked.clearToCastling(clicked, cell)){
                        const XY = {kingX: 6, rookX: 5}
        ...
        <div className='board'>
            ...
            {board.cells.map((row, id) => 
                <React.Fragment key={id}>
                    {row.map(element => 
                        <CellComponent clickedCell={clicked} click={click} selected={element.x === clicked?.x && element.y === clicked?.y} key={element.id} cell={element} />
                    )}
                </React.Fragment>
            )}
        </div>
