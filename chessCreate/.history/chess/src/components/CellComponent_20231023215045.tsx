...
const CellComponent:FC<CellProps> = ({cell, click, selected}) => {
    return (
        <div onClick={() => click(cell)} style={{backgroundColor: cell.available && cell.figure?.figureName !== FigureName.KING && cell.figure ? 'green' : ''}} className={['cell', cell.color, selected && cell.figure ? 'selected' : ''].join(' ')}>
            ...
            {cell.whitePawn3 && cell.figure?.color !== Color.WHITE && <div className='cell__block_black'></div>}
            {cell.whitePawn4 && cell.figure?.color !== Color.WHITE && <div className='cell__block_black'></div>}
            {cell.whitePawn5 && cell.figure?.color !== Color.WHITE && <div className='cell__block_black'></div>}
            {cell.whitePawn6 && cell.figure?.color !== Color.WHITE && <div className='cell__block_black'></div>}
            {cell.whitePawn7 && cell.figure?.color !== Color.WHITE && <div className='cell__block_black'></div>}
