...
const ChangeFigures:FC<ChangeFiguresProp> = ({array, pawn, setPawn, board, cell, king}) => {
    function getFigure(figure: Figure){
        ...
        setTimeout(() => {
            if (pawn.color === Color.WHITE) {
                if (cell.blackKingKill(pawn.cell, blackKing)) {
                    alertBlackKing();
                }
            }
            if (pawn.color === Color.BLACK) {
                if (cell.whiteKingKill(pawn.cell, whiteKing)) {
                    alertWhiteKing();
                }
            }
        }, 100);
