...
const BoardComponent:FC<BoardProps> = ({board, setBoard, change, currentColor}) => {
    ...
    const [blackKingDX, setBlackKingDX] = useState<number>({y: 0 , x: 0});
    const [blackKingDY, setBlackKingDY] = useState<number>({y: 0 , x: 0});

    const [whiteKingDX, setWhiteKingDX] = useState<number>({y: 0 , x: 0});
    const [whiteKingDY, setWhiteKingDY] = useState<number>({y: 0 , x: 0});
