            ...
            setTimeout(() => {
                if(cell.figure?.color === Color.WHITE){
                    if(cell.figure.figureName === FigureName.QUEEN){
                        if(cell.tryToKillKingHorizontal(cell, blackKing) || cell.tryToKillKingHorizontal(cell, blackKing) || cell.tryToKillKingDiagonal(cell, blackKing)){
                            alertBlackKing()
                        }
                    }
                    else if(cell.figure.figureName === FigureName.ROOK){
                        if(cell.tryToKillKingHorizontal(cell, blackKing) || cell.tryToKillKingHorizontal(cell, blackKing)){
                            alertBlackKing()
                        }
                    }
                    else if(cell.figure.figureName === FigureName.BISHOP){
                        if(cell.tryToKillKingDiagonal(cell, blackKing)){
                            alertBlackKing()
                        }
                    }
                    else if(cell.figure.figureName === FigureName.KNIGHT){
                        if(cell.tryToKillKingWithKnight(cell, blackKing)){
                            alertBlackKing()
                        }
                    }
                    else if(cell.figure.figureName === FigureName.PAWN){
                ...
                // if(cell.figure?.color === Color.BLACK){
                //     if(cell.tryToKillKing(cell, whiteKing)){
                //         alert('Шах білому королю!')
                //     }
                // }
            }, 100)
