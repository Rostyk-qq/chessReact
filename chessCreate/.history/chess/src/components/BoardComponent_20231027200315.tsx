...
const BoardComponent: FC<BoardProps> = ({
  ...
  const [blackBlockFigure, setBlackBlockFigure] = useState<Cell | null>(cell)
  let blackBlockFigure: Cell;
  ...
  const click = (cell: Cell) => {
    if (clicked && cell !== clicked && clicked.figure?.canMove(cell)) {
      ...
      if(cell.figure?.color === Color.WHITE){
        if(cell.blockBlackFigure(cell, blackKing)){
           const cellBlack: Cell = cell.blockBlackFigure(cell, blackKing)!
           cellBlack.block = true
           setBlackBlockFigure(cellBlack)
        }
      }  
